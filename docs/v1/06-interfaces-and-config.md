...
[02 — D1](02-architecture.md#key-design-decisions)).

| Tool | Input | Returns |
|---|---|---|
| `why` | `repo`, `file`, `line_start`, `line_end`, optional `question` ("why A instead of B?") | EvidenceBundle anchored on blame |
| `trace` | `ref` (SHA / PR# / issue# / URL / DocID) | full provenance neighborhood + body |
| `find_decision` | `query`, optional `repo` / `source` / `doc_type` / `since` | ranked evidence clusters |
| `history_of` | `path`, optional `repo`, `limit`, `before` | chronological timeline |
| `sync_now` | optional `source` | acknowledgment; errors if lock held |
| `sync_status` | — | last run per connector, cursor ages, lock state |
...
lore source add github|notion …    # append source config interactively
lore sync [--source=github] [--reembed]
lore status                        # sync state, doc/edge counts, lock state
lore why <file>:<L1>-<L2> ["question"] [--repo=…] [--explain | --raw]
lore ask "<question>" [filters…]   # find_decision, synthesized by default
lore trace <ref>
lore history <path>
...
Human-facing defaults: `why`/`trace`/`history` pretty-print the evidence chain
(tree with URLs); `--explain` and `lore ask` invoke SynthesisService (requires
LLM config); `--raw` emits bundle JSON for scripting.

## gRPC — programmatic API (`lore.v1`)

Not an MCP transport ([02 — D3](02-architecture.md#key-design-decisions)).
...
- `SyncService.Watch` streams sync progress — needed for a UI progress view
  and a good gRPC streaming showcase.
...
(synthesized answers), sync dashboard (`Watch` stream). No core changes
required — this is why the bundle/synthesis split exists on the gRPC surface.
...
repos:
  - path: ~/dev/myproject                  # local clone — blame/log ground truth
    remote: github:acme/myproject          # enrichment source (PRs, reviews, issues)
  - path: ~/dev/myproject-infra
    remote: github:acme/myproject-infra
...
Validation at load: unknown keys rejected; `token_env` variables must exist
when their source is configured; loopback/TLS rule enforced; embedder identity
checked against the index `meta`.

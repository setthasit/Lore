...
func rankMetas(metas ...entities.DocumentMeta) map[lore.DocID]entities.DocumentMeta {
...
func TestGraphRole(t *testing.T) {
	...
	tests := []struct {
		...
		{lore.DocType("message"), entities.RoleLinkedChange},
	}
...
func TestRankCitesSeedsAndReachedDocumentsDifferently(t *testing.T) {
	...
	commit.Type = lore.DocTypeCommit
...
func TestAssembleChains(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "nothing walked assembles no chains"},
		{
			...
			cited: map[lore.DocID]float32{"a": 0.3, "b": 0.2, "c": 0.1},
			want:  [][]lore.DocID{walkIDs("a", "b", "c")},
		},
		{
...
func citedNodes(scores map[lore.DocID]float32) []entities.EvidenceNode {
...
func TestAssembleChainsCopiesPathNodes(t *testing.T) {
	...
	path := rankPath(1, "a", "b")
	chains := assembleChains([]walkPath{path}, nil, citedNodes(map[lore.DocID]float32{"a": 1, "b": 0.5}))
...
func TestAssembleChainsCapsDenselyLinkedSeeds(t *testing.T) {
	...
	scores := make(map[lore.DocID]float32, seeds)
	ids := make([]lore.DocID, seeds)
	for i := range seeds {
		ids[i] = lore.DocID(fmt.Sprintf("seed-%d", i))
...
func TestRankReportsNoStandaloneGapForASeedInsideAChain(t *testing.T) {
	...
	if want := [][]lore.DocID{walkIDs("ticket", "page")}; !reflect.DeepEqual(got.Chains, want) {

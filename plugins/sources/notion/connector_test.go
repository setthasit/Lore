...
func (s *stub) connector(roots []string, opts ...Option) *Connector {
	return s.instanceConnector(defaultInstance, roots, opts...)
}

func (s *stub) instanceConnector(instance string, roots []string, opts ...Option) *Connector {
	base := []Option{withBatchSize(2), withMaxAttempts(3), withBackoff(time.Millisecond)}
	c := NewConnector(instance, fakeToken, roots, s.server.URL, append(base, opts...)...)
...
func TestDocumentMetadata(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(string(tt.id), func(t *testing.T) {
			...
			if d.Source != defaultInstance || d.Type != lore.DocTypePage {
...
func TestInstanceIDPrefixesDocumentIdentity(t *testing.T) {
	...
	s := newStub(t)
	got := drain(t, s.instanceConnector(instance, []string{rootPageID}), nil)
	if got.err != nil {
		t.Fatalf("Changes: %v", got.err)
	}

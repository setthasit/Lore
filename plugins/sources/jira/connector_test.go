...
func (s *stub) instanceConnector(instance string, projects []string, opts ...Option) *Connector {
	base := []Option{withBatchSize(2), withMaxAttempts(3), withBackoff(time.Millisecond)}
	c := NewConnector(instance, s.server.URL, fakeEmail, fakeToken, projects, append(base, opts...)...)
...
func TestJQLTemplate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c := NewConnector(defaultInstance, "https://acme.atlassian.net", fakeEmail, fakeToken, tt.projects)
...
func TestDocumentMetadata(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if d.Source != defaultInstance || d.Type != tt.docType {
			t.Errorf("%s: source %q type %q, want %q %q", tt.id, d.Source, d.Type, defaultInstance, tt.docType)
		}
		if d.RepoRef != "" {
...
func TestNewConnectorTrimsTheSiteRoot(t *testing.T) {
	c := NewConnector(defaultInstance, "https://acme.atlassian.net/", fakeEmail, fakeToken, nil)
	if got := c.Name(); got != defaultInstance {
		t.Errorf("Name() = %q, want %q", got, defaultInstance)
	}
...
func TestInstanceIDPrefixesDocumentIdentity(t *testing.T) {
	...
	s := newStub(t)
	c := s.instanceConnector(instance, fixtureProjects())
	if got := c.Name(); got != instance {
		t.Fatalf("Name() = %q, want %q", got, instance)
	}

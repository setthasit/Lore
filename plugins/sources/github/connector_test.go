...
func TestEveryDocumentIsFullyIdentified(t *testing.T) {
	...
	for _, d := range allDocs(got.batches) {
		...
		case d.Source != forgeName:
			t.Errorf("%s: source %q, want %q", d.ID, d.Source, forgeName)
		case d.Type == "":
...
func TestInstanceIDPrefixesIdentityButNotRepoRef(t *testing.T) {
	...
	c := NewConnector("github-acme", fakeToken, []string{fixtureRepo}, s.server.URL,
		withBatchSize(2), withMaxAttempts(3), withBackoff(time.Millisecond))
	...
	for _, d := range docs {
		...
		if !strings.HasPrefix(string(d.ID), "github-acme:") {
			t.Errorf("id %q is not prefixed with the instance id", d.ID)
		}
...
func TestMatchesRemote(t *testing.T) {
	...
	tests := []struct {
		...
		{remote: "github:/widgets", why: "empty namespace"},
		{remote: "github:other/thing", why: "not a configured repo"},
		{remote: "acme/widgets", why: "no forge prefix"},
		{remote: "", why: "empty remote"},
	}
	for _, tt := range tests {
...
func TestNewSourceBuildsTheConfiguredInstance(t *testing.T) {
	conn, err := Plugin().NewSource(lore.SourceConfig{
		Instance: "github-acme",
		Config:   []byte(`{"repos":["acme/widgets"]}`),
		Secrets:  map[string]string{"token": fakeToken},
	})
	if err != nil {
		t.Fatalf("NewSource: %v", err)
	}
	if got := conn.Name(); got != "github-acme" {
		t.Errorf("Name() = %q, want %q", got, "github-acme")
	}
	if !conn.(*Connector).MatchesRemote("github:acme/widgets") {
		t.Error("the configured repo does not match its own remote")
	}
}

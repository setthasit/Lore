...
func (s *stub) connectorFor(projects []string, opts ...Option) *Connector {
	...
	c := NewConnector(instanceID, fakeToken, projects, s.server.URL, append(base, opts...)...)
...
func TestDocumentMetadata(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(string(tt.id), func(t *testing.T) {
			...
			if d.Source != instanceID {
				t.Errorf("Source = %q, want %q", d.Source, instanceID)
			}
...
func TestNoteIDsStripToTheirThread(t *testing.T) {
	...
	for id, want := range parents {
		external := strings.TrimPrefix(string(id), instanceID+":")
...
func TestANonDefaultInstanceIDPrefixesIdentityButNotRepoRef(t *testing.T) {
	...
	for _, d := range docs {
		...
		if !strings.HasPrefix(string(d.ID), "gitlab-acme:") {
			t.Errorf("ID = %q, want the instance id as its prefix", d.ID)
		}
		if d.RepoRef != repoRefAll {
			t.Errorf("RepoRef = %q, want %q", d.RepoRef, repoRefAll)
		}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := c.MatchesRemote(tt.remote); got != tt.want {
				t.Errorf("MatchesRemote(%q) = %v, want %v", tt.remote, got, tt.want)
			}
		})
	}
}
...
func TestNewConnectorResolvesTheInstanceRoot(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c := NewConnector(instanceID, fakeToken, []string{projectPath}, tt.baseURL)

...
func newTestServer(t *testing.T, handler func(w http.ResponseWriter, attempt int, req embedRequest)) *testServer {
	...
	ts.Server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if req.Model != testEmbedModel {
			t.Errorf("model = %q, want %q", req.Model, testEmbedModel)
		}
...
func TestEmbedRetriesRateLimitHonoringRetryAfter(t *testing.T) {
	...
	t.Run("delta seconds", func(t *testing.T) {
		ts := newTestServer(t, func(w http.ResponseWriter, attempt int, req embedRequest) {
			if attempt == 1 {
...
func TestEmbedRespectsContextCancelDuringBackoff(t *testing.T) {
	...
	e, err := NewEmbedder(fakeKey, testEmbedModel, ts.URL, testDims)
	if err != nil {
		t.Fatalf("NewEmbedder: %v", err)
	}
...
func TestNewEmbedderValidatesArguments(t *testing.T) {
	cases := []struct {
		...
		{name: "no api key", model: testEmbedModel, dims: testDims},
		{name: "no model", apiKey: fakeKey, dims: testDims},
		{name: "zero dims", apiKey: fakeKey, model: testEmbedModel},
		{name: "negative dims", apiKey: fakeKey, model: testEmbedModel, dims: -1},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if _, err := NewEmbedder(tc.apiKey, tc.model, "", tc.dims); err == nil {
				t.Error("NewEmbedder succeeded, want error")
			}
		})
	}
}

func TestNewEmbedderResolvesEndpoint(t *testing.T) {
	cases := []struct {
		baseURL string
		want    string
	}{
	...
	for _, tc := range cases {
		e, err := NewEmbedder(fakeKey, testEmbedModel, tc.baseURL, testDims)
		if err != nil {
			t.Fatalf("NewEmbedder(%q): %v", tc.baseURL, err)
		}

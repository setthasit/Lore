...
func TestEmbedRejectsDimensionMismatch(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !strings.Contains(err.Error(), testEmbedModel) {
...
func TestEmbedRespectsContextCancel(t *testing.T) {
	...
	t.Run("cancelled during backoff", func(t *testing.T) {
		...
		e, err := NewEmbedder(testEmbedModel, ts.URL, testDims)
		if err != nil {
			t.Fatalf("NewEmbedder: %v", err)
		}
...
func TestNewEmbedderValidatesArguments(t *testing.T) {
	cases := []struct {
		...
		{name: "zero dims", model: testEmbedModel},
		{name: "negative dims", model: testEmbedModel, dims: -1},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if _, err := NewEmbedder(tc.model, "", tc.dims); err == nil {
...
func TestNewEmbedderResolvesEndpoint(t *testing.T) {
	...
	for _, tc := range cases {
		e, err := NewEmbedder(testEmbedModel, tc.baseURL, testDims)
		if err != nil {
